...
impl<I2C, E> STUSB4500Nvm<'_, I2C>
    ...
    pub(crate) fn unlock(inner: &mut STUSB4500<I2C>) -> Result<STUSB4500Nvm<I2C>, Error<E>> {
        ...
        inner.write(Register::NvmCtrl0, (NvmCtrl0::Power | NvmCtrl0::Enable).bits())?;
    ...
    pub fn lock(self) -> Result<(), Error<E>> {
        self.inner.write(Register::NvmCtrl0, NvmCtrl0::Enable.bits())?;
    ...
    fn issue_request_with_sector(&mut self, sector: u8) -> Result<(), Error<E>> {
        self.inner.write(Register::NvmCtrl0, sector |
            (NvmCtrl0::Power | NvmCtrl0::Enable | NvmCtrl0::Request).bits())?;
    ...
    fn read_sector(&mut self, sector: u8) -> Result<[u8; 8], Error<E>> {
        self.inner.write(Register::NvmCtrl1, NvmCtrl1Opcode::ReadSector as u8)?;
        self.issue_request_with_sector(sector)?;

        let mut buf = [0x00; 8];
        self.inner.i2c.write(self.inner.address, &[Register::RWBuffer as u8])
            .map_err(|err| Error::I2CError(err))?;
        self.inner.i2c.read(self.inner.address, &mut buf).map_err(|err| Error::I2CError(err))?;
        Ok(buf)
    }
    ...
    fn write_sector(&mut self, sector: u8, data: &[u8; 8]) -> Result<(), Error<E>> {
        ...
        self.inner.i2c.write(self.inner.address, &buf).map_err(|err| Error::I2CError(err))?;
        self.inner.write(Register::NvmCtrl1, NvmCtrl1Opcode::LoadPlr as u8)?;
        self.issue_request()?;

        self.inner.write(Register::NvmCtrl1, NvmCtrl1Opcode::WriteSector as u8)?;
        self.issue_request_with_sector(sector)
    }

    fn erase_sectors(&mut self) -> Result<(), Error<E>> {
        self.inner.write(Register::NvmCtrl1, NvmCtrl1Opcode::LoadSer as u8 |
            (NvmCtrl1::EraseSector0 |
             NvmCtrl1::EraseSector1 |
             NvmCtrl1::EraseSector2 |
             NvmCtrl1::EraseSector3 |
             NvmCtrl1::EraseSector4).bits())?;
        self.issue_request()?;

        self.inner.write(Register::NvmCtrl1, NvmCtrl1Opcode::EraseSectors as u8)?;
        self.issue_request()
    }
}
